...
class ChartParser(nn.Module):
        ...
        with torch.no_grad():
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                ...
                p_i, p_j, p_label, p_augment, g_i, g_j, g_label \
                    = self.parse_from_annotations(fencepost_annotations_start[start:end,:], fencepost_annotations_end[start:end,:], sentences[i], golds[i])
    ...
    def parse_from_annotations(self, fencepost_annotations_start, fencepost_annotations_end, sentence, gold=None):
        ...
        if is_train:
            ...
            return self.decode_from_chart(sentence, label_scores_chart_np, arc_dc_np, type)
    ...
    def decode_from_chart(self, sentence, label_scores_chart_np, arc_dc_np, type, gold=None):
